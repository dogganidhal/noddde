...
A concert venue where customers reserve specific seats. Multiple customers attempt to reserve the same seat concurrently. This example demonstrates **pessimistic concurrency with advisory locks** — the right strategy when command handlers involve expensive validation and wasted retries are costly.

> **Note**: This is a conceptual walkthrough. For a runnable concurrency example, see the [Flash Sale sample](https://github.com/dogganidhal/noddde/tree/main/samples/sample-flash-sale) which demonstrates both optimistic (`main-optimistic.ts`) and pessimistic (`main-pessimistic.ts`) concurrency strategies.
