...
export const onPaymentCompleted = async (
  ...
  const availableRooms = (await ports.queryBus.dispatch({
...
The third saga is simpler: it starts on `GuestCheckedIn`, sends a welcome SMS via `smsService`, and sends a farewell SMS on `GuestCheckedOut`. It demonstrates ports side effects from a saga without dispatching any commands.
...
`SearchAvailableRoomsHandler` is registered as a standalone query handler rather than a projection-backed query. It reads directly from `roomAvailabilityViewStore` via ports injection:
...
The handler is then consumed inside `BookingFulfillmentSaga` via `ports.queryBus.dispatch(...)` — the same query bus that HTTP clients use. This is how the saga finds an available room after payment completes.

See [Queries](/docs/read-model/queries) for the full standalone query handler pattern.

## Domain Configuration

The domain configuration uses `defineDomain` to capture the pure domain structure, then `wireDomain` to connect adapters.
...
```ts
// adapters/http/routes/bookings.ts
