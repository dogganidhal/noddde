...
The remaining decide handlers (`FailPayment`, `RefundPayment`, `CreateBooking`, `ConfirmBooking`, `CancelBooking`, `ModifyBooking`) follow the same pattern — each in its own file, each typed with `InferDecideHandler<BookingTypes, "CommandName">`.
...
export const Booking = defineAggregate<BookingTypes>({
...
- [Persistence Adapters](/docs/running/persistence-adapters) — the `@noddde/drizzle` adapter used in this sample
