...
The `Booking` aggregate is the most instructive of the three. Its status field is a strict state machine: every payment command validates the current status before emitting an event. All handlers are extracted to separate files typed with `InferCommandHandler` and `InferApplyHandler`.

#### Command handlers (extracted)

```ts
// command-handlers/handle-request-payment.ts
...
```ts
// command-handlers/handle-complete-payment.ts
import type { InferCommandHandler } from "@noddde/core";
import type { BookingDef } from "../booking";
...
The remaining command handlers (`FailPayment`, `RefundPayment`, `CreateBooking`, `ConfirmBooking`, `CancelBooking`, `ModifyBooking`) follow the same pattern — each in its own file, each typed with `InferCommandHandler<BookingDef, "CommandName">`.

#### Apply handlers (extracted)

```ts
// apply-handlers/apply-payment-requested.ts
...
```ts
// apply-handlers/apply-payment-failed.ts
import type { InferApplyHandler } from "@noddde/core";
import type { BookingDef } from "../booking";
...
Notice that `PaymentFailed` resets status back to `"pending"` rather than `"cancelled"`. This lets the guest retry payment without creating a new booking. The other apply handlers (`PaymentCompleted`, `BookingCreated`, `BookingConfirmed`, `BookingCancelled`, `BookingModified`, `PaymentRefunded`) follow the same pattern.
...
```ts
// transition-handlers/on-booking-created.ts
...
```ts
// transition-handlers/on-payment-completed.ts
import type { BookingFulfillmentState } from "../state";
...
```ts
// transition-handlers/on-booking-cancelled.ts
import type { BookingFulfillmentState } from "../state";
...
The `PaymentFailed`, `BookingConfirmed`, `BookingModified`, `PaymentRequested`, and `PaymentRefunded` handlers follow the same pattern — each in its own file under `transition-handlers/`.
...
```ts
// transition-handlers/on-payment-requested.ts
