...
A flash sale where limited stock sells first-come-first-served. Multiple buyers purchase the same item concurrently. This sample demonstrates both **optimistic concurrency with retry** (Drizzle + PostgreSQL) and **pessimistic concurrency with advisory locks** (Prisma + MySQL) — showing when to choose each strategy.
...
**Stack**: Drizzle + PostgreSQL (optimistic) | Prisma + MySQL (pessimistic)
