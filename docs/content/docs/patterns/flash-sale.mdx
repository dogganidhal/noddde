...
The event bus uses `@noddde/nats` (`NatsEventBus`) with JetStream enabled, started automatically via TestContainers alongside PostgreSQL. Set `EVENT_BUS=in-memory` to skip the NATS container and use `EventEmitterEventBus` instead. See [Event Bus Adapters](/docs/running/event-bus-adapters) for the full adapter reference.
