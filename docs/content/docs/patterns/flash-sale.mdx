...
The aggregate uses extracted handlers — each command handler and apply handler lives in its own file, typed with `InferCommandHandler` and `InferApplyHandler`.

### Command handlers (extracted)

```ts
// command-handlers/handle-create-flash-sale.ts
...
```ts
// command-handlers/handle-purchase-item.ts
import type { InferCommandHandler } from "@noddde/core";
import type { FlashSaleItemTypes } from "../flash-sale-item";
...
```ts
// apply-handlers/apply-flash-sale-created.ts
...
```ts
// apply-handlers/apply-item-purchased.ts
import type { InferApplyHandler } from "@noddde/core";
import type { FlashSaleItemTypes } from "../flash-sale-item";
...
The `PurchaseRejected` apply handler follows the same pattern — it returns `state` unchanged (no-op).
