...
This example walks through the fund transfer sample included with noddde. It demonstrates the [Unit of Work](/docs/domain-configuration/unit-of-work) pattern for grouping multiple commands into a single atomic boundary — both succeed or neither takes effect.
...
With a database-backed [ORM adapter](/docs/infrastructure/orm-adapters), the `commit()` wraps operations in a real database transaction (`BEGIN`/`COMMIT`/`ROLLBACK`), giving you true ACID guarantees.
...
- [Unit of Work](/docs/domain-configuration/unit-of-work) — Full reference for `withUnitOfWork()` and the `UnitOfWork` interface
