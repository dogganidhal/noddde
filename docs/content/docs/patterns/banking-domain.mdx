...
A projection builds a query-optimized view from events using `defineProjection`. Each event gets its own reducer — no switch statements:
...
The `reducers` map provides one handler per event name — the framework narrows the event type for each key. Events that don't affect the view return it unchanged (pass-through). Query handlers are co-located with the projection that builds the view they serve.
