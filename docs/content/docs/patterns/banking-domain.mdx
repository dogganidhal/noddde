...
This example walks through a banking domain to demonstrate commands, events, aggregates, projections, queries, infrastructure, and domain configuration working together.

> **Note**: This is a conceptual walkthrough. For runnable projection examples, see the [Auction sample](https://github.com/dogganidhal/noddde/tree/main/samples/sample-auction) (projections + ViewStore) or the [Hotel Booking sample](https://github.com/dogganidhal/noddde/tree/main/samples/sample-hotel-booking) (multiple projections with custom view stores).
...
A projection builds a query-optimized view from events using `defineProjection`. Each event gets its own handler in the `on` map — no switch statements:
...
export const BankAccountProjection = defineProjection<BankAccountProjectionDef>({
  on: {
    ...
    // TransactionProcessed and TransactionDeclined not needed — omitted
  },
  queryHandlers: {
...
The `on` map provides one handler per event — the framework narrows the event type for each key. Only events that affect the view need entries; `TransactionProcessed` and `TransactionDeclined` are omitted since they don't change this view. Query handlers are co-located with the projection that builds the view they serve.

## Step 7: Define, Wire, and Run
