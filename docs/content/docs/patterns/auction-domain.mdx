...
This example walks through the auction domain sample. It demonstrates infrastructure injection via the Clock pattern, multi-validation command handlers, rejection events, no-op apply handlers, event upcasting, and projections with ViewStore.
...
The recommended pattern extracts command handlers and apply handlers into separate files, typed with `InferCommandHandler` and `InferApplyHandler`. The aggregate definition then imports and wires them together.

### Command handlers (extracted)

Each command handler is a standalone, independently testable function. The type is inferred from the aggregate's type bundle (`AuctionDef`) and the command name.

```ts
// command-handlers/handle-create-auction.ts
...
The `CloseAuction` handler follows the same pattern. Each file imports only `InferCommandHandler` and the type bundle — no runtime dependencies beyond the function itself.

### Apply handlers (extracted)

Apply handlers are also extracted. They are pure functions — no infrastructure access, no async.

```ts
// apply-handlers/apply-auction-created.ts
...
```ts
// apply-handlers/apply-bid-placed.ts
import type { InferApplyHandler } from "@noddde/core";
import type { AuctionDef } from "../auction";
...
The `AuctionClosed` apply handler follows the same pattern.
