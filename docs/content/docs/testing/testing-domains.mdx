...
type DomainSpy = {
  ...
  dispatchedCommands: Command[]; // All commands dispatched, whatever the outcome
  unhandledCommands: Command[]; // Commands with no registered handler (suppressed)
  commandErrors: Array<{ command: Command; error: Error }>; // Handler threw -- recorded and rethrown
};
```
...
Commands dispatched by sagas that have no registered handler are captured by the spy but do not throw. This lets you test saga orchestration without wiring every downstream aggregate. They also land in `spy.unhandledCommands`, so you can assert on the reaction even though nothing handled it:

```ts title="__tests__/unhandled-commands.test.ts"
...
A missing handler is the _only_ dispatch failure the harness suppresses. If a command's handler runs and throws -- a `decide` handler rejecting a business rule, say -- the error is recorded on `spy.commandErrors` **and** rethrown, so it never vanishes:

```ts title="__tests__/command-errors.test.ts"
const { domain, spy } = await testDomain({
...
expect(spy.commandErrors[0]!.error.message).toBe("cannot touch a locked thing");
```

Errors from saga reaction commands are the one case that does not surface as a rejected promise -- the event bus isolates and logs handler errors rather than rejecting -- so `spy.commandErrors` is how you assert on them.
