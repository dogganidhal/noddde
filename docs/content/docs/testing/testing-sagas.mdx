---
title: Testing Sagas
description: Using testSaga for Given-When-Then saga testing with automatic CQRS setup.
---
...
| `.withPorts(infra)`    | Custom ports. Defaults to `{}`.                                              |
| `.withCQRSPorts(cqrs)` | Override the default no-op CQRS buses.                                       |
...
## Automatic CQRS Ports

Saga handlers receive `TPorts & CQRSPorts` -- they have access to `commandBus`, `eventBus`, and `queryBus`. The harness automatically provides no-op implementations so you do not need to wire mock buses for most tests.
...
When saga handlers use domain-specific ports (notification services, APIs, etc.), provide it via `.withPorts()`:
...
const result = await testSaga(NotificationSaga)
  ...
  .withPorts({ notificationService: mockNotifier })
  .execute();
