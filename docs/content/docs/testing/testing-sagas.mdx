...
Each `on` entry in a saga bundles an `id` function that extracts the saga instance ID from an event. These are plain functions -- test them directly:

```ts
const sagaId = OrderFulfillmentSaga.on.OrderPlaced!.id({
  name: "OrderPlaced",
  payload: { orderId: "o-1", amount: 99.99 },
});
...
describe("on entry id extraction", () => {
  it("should extract orderId from OrderPlaced", () => {
    ...
    expect(OrderFulfillmentSaga.on.OrderPlaced!.id(event)).toBe(
      "order-1",
    );
  });
  ...
  it("should extract referenceId from PaymentCompleted", () => {
    ...
    expect(OrderFulfillmentSaga.on.PaymentCompleted!.id(event)).toBe(
      "order-1",
    );
  });
});
...
Replay a sequence of events through the saga to verify the complete lifecycle. You can either use the harness for each step or call handlers directly via the `on` map:
...
it("should complete the full happy path", async () => {
  ...
  // Step 1: Order placed
  ({ state, commands } = OrderFulfillmentSaga.on.OrderPlaced!.handle(
    {
      name: "OrderPlaced",
      payload: {
        orderId: "o-1",
  ...
  ({ state, commands } = OrderFulfillmentSaga.on.PaymentCompleted!.handle(
    {
  ...
  ({ state, commands } = OrderFulfillmentSaga.on.ShipmentDispatched!.handle(
    {
...
- [Testing Aggregates and Projections](/docs/testing/testing-aggregates-and-projections) -- `testAggregate` for command handler testing
