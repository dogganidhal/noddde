...
| `.withPorts(infra)` | Custom ports for the handler. Defaults to `{}`.                                                   |
...
Pass ports via `.withPorts()`. This is how you inject clocks, loggers, or any domain-specific dependency:
...
For ports with multiple members, create a reusable mock factory to keep tests concise:
...
const result = await testAggregate(BankAccount)
  ...
  .withPorts(createMockBankingInfrastructure())
  .execute();
```
