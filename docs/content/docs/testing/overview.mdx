...
For end-to-end coverage that goes through real wiring, drive the runtime directly: `defineDomain` from `@noddde/core` + `wireDomain` from `@noddde/engine` with whichever persistence adapter and event bus match production. Dispatch commands and assert on the resulting state, projection views, or saga reactions exactly as your service would. See [Testing Domains](/docs/testing/testing-domains) for the recommended harness over this layer.
...
| -------------- | --------------------------------------- | -------------- | ------------------------------- |
| **Aggregate**  | Prior events (replayed through evolve)  | Command        | Produced events + new state     |
