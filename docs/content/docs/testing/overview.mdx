...
```ts
// Command handler — import the extracted function
import { handleAuthorizeTransaction } from "./command-handlers/handle-authorize-transaction";

const events = handleAuthorizeTransaction(command, state, infrastructure);

// Apply handler — import the extracted function
import { applyTransactionAuthorized } from "./apply-handlers/apply-transaction-authorized";

const newState = applyTransactionAuthorized(payload, state);
