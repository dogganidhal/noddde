...
await domain.dispatchCommand({ name: "CreateBankAccount", targetAggregateId: "acc-1" });
...
These are particularly useful when testing [saga workflows](/docs/process-managers/overview), where correlation should propagate across aggregate boundaries.
