...
The interceptor needs the `Domain` instance, so it is registered through DI rather than constructed inline. `NodddeMetadataInterceptor.withExtractor(extractor)` returns a provider that resolves `Domain` for you — add it to the `providers` of the module that declares the controller, then reference the class in `@UseInterceptors()`:

```ts title="order.module.ts"
import { Module } from "@nestjs/common";
import type { ExecutionContext } from "@nestjs/common";
import { NodddeMetadataInterceptor } from "@noddde/nestjs";
import { OrderController } from "./order.controller";

const extractMetadata = (ctx: ExecutionContext) => {
...
@Module({
  ...
  providers: [NodddeMetadataInterceptor.withExtractor(extractMetadata)],
})
...
@Controller("orders")
@UseInterceptors(NodddeMetadataInterceptor)
export class OrderController {
...
To apply globally, register the same provider and alias `APP_INTERCEPTOR` to it with `useExisting` so both refer to one instance:
...
- **Outbox relay**: Unless `startOutboxRelay: false` is set, `domain.startOutboxRelay()` runs on application bootstrap and `domain.stopOutboxRelay()` runs before `domain.shutdown()`. Both calls are no-ops when no outbox is configured, so the default is safe regardless of your wiring.
...
| `startOutboxRelay` | `boolean`          | No       | `true`  | Start the outbox relay on bootstrap and stop it before shutdown |
