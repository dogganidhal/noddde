...
For state-stored aggregates that need a domain-shaped table instead of an opaque JSON column, all three ORM adapters expose `stateStored(table, { mapper })` — the mapper owns the row shape, the framework owns id and version. See [Per-Aggregate Dedicated State Tables](/docs/running/persistence-adapters#per-aggregate-dedicated-state-tables) and [Why an Aggregate State Mapper?](/docs/design-decisions/why-state-mapper).
...
noddde offers three concurrency modes for aggregate persistence, configured via the `concurrency` field in aggregate wiring. All modes work with every database dialect -- only pessimistic locking has [dialect restrictions](/docs/running/persistence-adapters#dialect-support-matrix).
...
const domain = await wireDomain(myDomain, {
  aggregates: {
    ...
    BankAccount: {
      ...
      concurrency: {
        strategy: "pessimistic",
        locker: new DrizzleAdvisoryLocker(db, "pg"),
      },
    },
...
SQLite does not support advisory locks -- use `InMemoryAggregateLocker` for single-process SQLite deployments. See the [Dialect Support Matrix](/docs/running/persistence-adapters#dialect-support-matrix) for full details.
...
Inside an explicit `withUnitOfWork()`, optimistic `ConcurrencyError` is never retried -- it propagates immediately. Pessimistic locking acquires the lock before the command's load phase and holds it until the **owning** unit of work settles (commit or rollback), not merely until that command's handler returns -- the write itself happens at the owning UoW's commit, so the lock has to outlive the individual command to serialize access at all. A second command targeting the same aggregate therefore queues (or times out per `lockTimeoutMs`) until the enclosing `withUnitOfWork()` call has committed or rolled back.
...
The same aggregate definition works with either persistence strategy. The `defineAggregate` function does not know or care about persistence -- it defines `initialState`, `decide`, and `evolve` regardless.
...
For production-ready persistence with transaction support, consider the [Persistence Adapters](/docs/running/persistence-adapters) packages instead of writing custom implementations.
...
For database-backed unit of work implementations using real transactions, see the [Persistence Adapters](/docs/running/persistence-adapters) packages.

## Next Steps

- [Persistence Adapters](/docs/running/persistence-adapters) -- Move from in-memory to production using Drizzle, Prisma, or TypeORM
- [Drizzle](/docs/persistence/drizzle), [Prisma](/docs/persistence/prisma), or [TypeORM](/docs/persistence/typeorm) -- Per-adapter setup for whichever ORM your project uses
