...
In distributed systems, commands may be delivered more than once — network retries, at-least-once messaging, or user double-clicks can all produce duplicates. **Idempotent command processing** ensures that a command with a given `commandId` is executed at most once.

### How It Works

1. The caller adds a `commandId` to the command object.
2. Before executing the command, the domain checks the `IdempotencyStore` — if the `commandId` has already been processed, the command is silently skipped.
3. After successful execution, the `commandId` is recorded in the same unit of work as the events, ensuring atomicity.

```ts
// Dispatch with a commandId — safe to retry
await domain.dispatchCommand({
  name: "Transfer",
  targetAggregateId: "acc-123",
  payload: { amount: 100, toAccountId: "acc-456" },
  commandId: "transfer-req-789", // idempotency key
});

// Second dispatch with the same commandId — no-op
await domain.dispatchCommand({
  name: "Transfer",
  targetAggregateId: "acc-123",
  payload: { amount: 100, toAccountId: "acc-456" },
  commandId: "transfer-req-789", // already processed, silently skipped
});
```

Commands without a `commandId` bypass idempotency entirely and are always processed.

### Configuring Idempotency

Add an `idempotencyStore` factory to the infrastructure section:
...
The `InMemoryIdempotencyStore` accepts an optional `ttlMs` parameter for automatic cleanup of expired records in `exists()` checks:
...
For manual cleanup of expired records, call `removeExpired(ttlMs)`:
...
Implement this interface for production backends (PostgreSQL, Redis, etc.) to make idempotency durable across process restarts.
