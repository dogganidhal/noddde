...
```
2026-03-28T12:00:00.000Z  WARN [noddde:domain] Using in-memory aggregate persistence. This is not suitable for production.
2026-03-28T12:00:00.000Z  WARN [noddde:domain] Using in-memory CQRS buses. This is not suitable for production.
2026-03-28T12:00:00.000Z  WARN [noddde:domain] Using in-memory saga persistence. This is not suitable for production.
```
...
By default, a `StructuredLogger` at `'warn'` level is used. It **auto-detects the environment** and picks the right output format:

- **Development** (interactive terminal): colored, human-readable output
  ```
  2026-03-28T12:00:00.000Z  WARN [noddde:domain] Using in-memory CQRS buses.
  ```
- **Production** (`NODE_ENV=production`, containers, CI, piped output): newline-delimited JSON (NDJSON)
  ```json
  {
    "timestamp": "2026-03-28T12:00:00.000Z",
    "level": "warn",
    "namespace": "noddde:domain",
    "message": "Using in-memory CQRS buses."
  }
  ```

Detection logic: pretty mode is used when `NODE_ENV` is not `'production'` **and** `stdout` is a TTY. Everything else gets JSON -- no configuration needed.
...
const domain = await wireDomain(bankingDomain, {
  logger: new StructuredLogger("debug"), // Show all framework messages
  // ...other wiring
});
```
