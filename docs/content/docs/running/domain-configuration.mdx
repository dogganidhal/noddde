...
<Callout type="info">
  A `UnitOfWork` groups multiple persistence operations into a single atomic
  transaction with deferred event publishing — events emitted during the work
  are held until the work commits, then dispatched together. See [Unit of
  Work](/docs/running/persistence#unit-of-work) for the full contract.
</Callout>
...
See [Persistence Adapters](/docs/running/persistence-adapters) for adapter-specific setup.
...
projections: {
  BankAccount: {
    viewStore: new PrismaBankAccountViewStoreFactory(prisma),
  },
  TransactionHistory: {
    viewStore: new InMemoryViewStoreFactory(),
  },
  // Or wrap a builder function:
  Audit: {
    viewStore: createViewStoreFactory((ctx) => new AuditViewStore(ctx)),
  },
},
```

The factory's `getForContext(ctx?)` is called per strong-consistency read-modify-write so the view store participates in the active transaction. For eventual-consistency reads and query handlers, the framework calls `getForContext(undefined)` once and caches the result. See [View Persistence](/docs/read-model/view-persistence) for details.
...
Each entry is a bare projection definition. View stores are wired separately in `wireDomain`'s `projections` field (see [Projection Wiring](#projection-wiring) above). When a `viewStore` factory is wired for a projection, the engine auto-persists views using the `id` and `reduce` functions in the projection's `on` map. See [View Persistence](/docs/read-model/view-persistence) for details.
