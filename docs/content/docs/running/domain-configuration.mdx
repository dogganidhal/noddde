...
description: Using defineDomain and wireDomain to separate domain structure from runtime ports, creating a running Domain instance.
---

The domain configuration is the entry point of every noddde application. It wires together your write model (aggregates), read model (projections), process model (sagas), and adapters into a single `Domain` instance that can dispatch commands and queries.

The API is split into two phases:

- **`defineDomain`** -- Captures the pure domain structure (aggregates, projections, sagas, handlers) as a sync identity function. No ports, no async, no side effects.
- **`wireDomain`** -- Connects that definition to adapters (persistence, buses, concurrency, snapshots) and returns a running `Domain` instance.
...
The `TPorts` generic is a type parameter only -- it flows through handler signatures for type safety but no ports value is present in the definition.
...
```ts
const domain = await wireDomain(bankingDomain, {
  // User-provided services (what handlers receive as `ports`)
  adapters: () => ({
...
When adapter components fall back to in-memory defaults, the framework logs startup warnings via the configured logger. In development (TTY), you'll see colored output:
...
| Field              | Description                                                                              |
| ------------------ | ---------------------------------------------------------------------------------------- |
| `adapters`         | Factory for user-provided services. What handlers receive as the `ports` parameter.      |
...
| `buses`            | Factory for CQRS buses. Receives resolved user adapters.                                 |
...
The framework uses a `Logger` interface for all internal logging. By default, a `NodddeLogger` at `'warn'` level is used -- it auto-detects the environment (colored output in dev, NDJSON in production). The logger is also automatically available as `ports.logger` in all handlers, and passed to the `adapters` factory so your custom services can use it too.

See [Logging](/docs/running/logging) for full details on output formats, log levels, using the logger in handlers and custom ports, and bringing your own logger.
...
projections: {
  BankAccount: {
    viewStore: (adapters) => infra.bankAccountViewRepository,
  },
  TransactionHistory: {
    viewStore: (adapters) => new InMemoryViewStore(),
  },
},
```

The `viewStore` factory receives the resolved user adapters, allowing you to pull view stores from your adapter services.
...
Standalone command handlers receive the merged ports (`TPorts & CQRSPorts`), giving them access to all three buses. This makes them suitable for orchestration tasks such as batch processing, system commands, and scheduled operations.
...
readModel: {
  projections: {
    ...
    BankAccount: {
      ...
      viewStore: (adapters) => infra.bankAccountViewStore,
    },
...
processModel: {
  ...
  standaloneEventHandlers: {
    BookingConfirmed: (event, ports) => {
      ports.emailService.sendConfirmation(event.payload.guestEmail);
    },
  },
},
```
...
standaloneEventHandlers: {
  OrderPlaced: async (event, ports) => {
    await ports.analytics.track("order_placed", event.payload);
  },
  PaymentReceived: (event, ports) => {
    ports.logger.info(`Payment received: ${event.payload.amount}`);
  },
},
```

Each handler receives the full event object (including metadata) and the domain ports. Use a saga instead when you need state tracking, command dispatch, or multi-step coordination.
...
The `adapters` getter returns the merged custom adapters and CQRS buses.

### Reading Projection Views

Projection views are persisted via `ViewStore`. To read a projection's view, query through the query bus or access the view store directly from your adapters:

```ts
// Via query bus
const view = await domain.adapters.queryBus.dispatch({
  name: "GetOrderSummary",
  payload: { orderId: "order-1" },
});

// Or via the view store on your ports
const view = await domain.adapters.orderSummaryViewStore.load("order-1");
```
...
// Step 2: Wire with adapters (async)
const domain = await wireDomain(bankingDomain, {
  adapters: () => ({
    bankAccountViewRepository: new InMemoryBankAccountViewRepository(),
  ...
  projections: {
    ...
    TransactionHistory: {
      viewStore: (adapters) => infra.transactionViewRepository,
    },
  },
  buses: () => ({
    commandBus: new InMemoryCommandBus(),
...
4. **Auto-close adapters** — The domain scans all registered adapter components and calls `close()` on any that implement the `Closeable` interface.
...
### Making Ports Closeable

If your custom adapters hold resources (database connections, file handles, network sockets), implement the `Closeable` interface so the domain releases them automatically:
...
The domain auto-detects `Closeable` implementations across all adapters you provide — custom adapter values, buses, persistence stores, and ORM adapters. No additional configuration is needed.

`close()` must be idempotent: calling it more than once should have no additional effect.

## Next Steps

- [Ports](/docs/running/ports) -- The ports and adapters providers in detail
