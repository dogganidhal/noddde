...
- **`idempotencyStore`** -- Factory function returning an `IdempotencyStore` for deduplicating commands. When configured, commands with a `commandId` field are checked for duplicates before execution. See [Idempotent Commands](/docs/running/idempotent-commands).
