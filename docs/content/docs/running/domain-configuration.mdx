...
When infrastructure components fall back to in-memory defaults, the framework logs startup warnings via the configured logger. With the default `StructuredLogger`, output is NDJSON:

```json
{"timestamp":"2026-03-28T12:00:00.000Z","level":"warn","namespace":"noddde:domain","message":"Using in-memory aggregate persistence. This is not suitable for production."}
{"timestamp":"2026-03-28T12:00:00.000Z","level":"warn","namespace":"noddde:domain","message":"Using in-memory CQRS buses. This is not suitable for production."}
{"timestamp":"2026-03-28T12:00:00.000Z","level":"warn","namespace":"noddde:domain","message":"Using in-memory saga persistence. This is not suitable for production."}
```
...
| `logger`           | Framework logger instance. Defaults to `StructuredLogger` at `'warn'` level.                 |

All fields are optional. Both `wireDomain(definition)` and `wireDomain(definition, {})` use in-memory defaults for everything.

### Logging

The framework uses a `Logger` interface for all internal logging -- domain initialization, command dispatch, saga execution, concurrency control, and outbox processing.

By default, a `StructuredLogger` at `'warn'` level is used. It emits newline-delimited JSON (NDJSON) to `process.stdout` (debug/info) and `process.stderr` (warn/error), making it suitable for production environments, container deployments, and log aggregators (Datadog, ELK, CloudWatch).

To see more detail, provide a logger with a lower level:

```ts
import { wireDomain, StructuredLogger } from "@noddde/engine";

const domain = await wireDomain(bankingDomain, {
  logger: new StructuredLogger("debug"), // Show all framework messages as NDJSON
  // ...other wiring
});
```

Available log levels (ordered by severity): `'debug'`, `'info'`, `'warn'`, `'error'`, `'silent'`.

For human-readable console output during development, use `ConsoleLogger`:

```ts
import { wireDomain, ConsoleLogger } from "@noddde/engine";

const domain = await wireDomain(bankingDomain, {
  logger: new ConsoleLogger("debug"), // Human-readable [namespace] prefixed output
});
```
...
You can also provide your own logger by implementing the `Logger` interface from `@noddde/core`:
...
const pinoLogger: Logger = {
  ...
  child: (ns) => createPinoChild(ns),
};
```

The framework creates child loggers with namespaces like `command`, `saga`, `domain`, `concurrency`, and `outbox` to scope log output by component.
