...
- **`idempotencyStore`** -- Factory function returning an `IdempotencyStore` for deduplicating commands. When configured, commands with a `commandId` field are checked for duplicates before execution. See [Idempotent Command Processing](/docs/running/persistence#idempotent-command-processing).
