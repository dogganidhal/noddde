...
For production, all three [persistence adapter packages](/docs/running/persistence-adapters) (`@noddde/drizzle`, `@noddde/prisma`, `@noddde/typeorm`) include a database-backed `OutboxStore` that participates in the same database transaction as your aggregate persistence:
...
- [Persistence Adapters](/docs/running/persistence-adapters) -- Production persistence with database transactions
