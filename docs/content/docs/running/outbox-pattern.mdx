...
Add the `outbox` field to your domain wiring:
...
For production, all three [ORM adapter packages](/docs/running/orm-adapters) (`@noddde/drizzle`, `@noddde/prisma`, `@noddde/typeorm`) include a database-backed `OutboxStore` that participates in the same database transaction as your aggregate persistence:
