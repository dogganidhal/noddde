...
The **transactional outbox pattern** solves this by writing events to an outbox table _within the same transaction_ as aggregate persistence. A background relay polls for unpublished entries and dispatches them, guaranteeing **at-least-once delivery**.
...
The outbox guarantees events are delivered **at least once**, not exactly once. In the happy path, events are dispatched immediately after commit _and_ marked as published. But if the node crashes:
...
The guarantee is scoped to what the event bus reports back to the relay: an entry is marked published only when `dispatch()` resolves without throwing. A broker-backed bus that fails to deliver rejects, so the relay retries the entry on the next poll. An in-process bus that isolates handler errors and always resolves (such as `EventEmitterEventBus`) never reports a handler failure, so the entry is marked published anyway — at-least-once then holds at the transport level, not against in-process handler failures.

A transient failure while polling (for example a dropped database connection in `loadUnpublished`) is logged at `error` level and treated as "this poll did nothing"; the relay keeps its timer and retries on the next tick.
...
- [Idempotent Commands](/docs/running/idempotent-commands) -- The companion pattern for safe at-least-once consumers
- [Event Bus Adapters](/docs/running/event-bus-adapters) -- Configuring the broker the outbox relay publishes to
