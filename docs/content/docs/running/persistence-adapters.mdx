...
#### Migrating from `columns: { ... }`

Earlier releases accepted a `columns` map alongside the table. That field has been removed; pass `{ mapper: jsonStateMapper(...) }` to keep the same on-disk shape, or supply a typed mapper to spread state across dedicated columns. Each adapter spec's `## Migration` section in the repository (`specs/drizzle/`, `specs/prisma/`, `specs/typeorm/`) shows the before/after for every variant.
