...
By default, noddde creates a `StructuredLogger` at `'warn'` level. It **auto-detects the environment** and picks the right output format:
...
The default level is `'warn'` -- only warnings and errors are visible. To see more detail, pass a `StructuredLogger` with a lower level:
