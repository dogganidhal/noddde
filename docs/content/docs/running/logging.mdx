---
title: Logging
description: How noddde logs internally, how to configure log levels, and how to use the framework logger in your own handlers and adapters.
---

noddde has a built-in structured logging system that works out of the box with zero configuration. The framework logs its own lifecycle events (domain init, command dispatch, saga execution, concurrency, outbox) and makes the same logger available to your handlers and custom adapters.
...
The framework logger is automatically available as `ports.logger` in **every handler** -- aggregate decide handlers, event handlers, saga handlers, query handlers, and standalone handlers.
...
const BankAccount = defineAggregate<BankAccountTypes>({
  ...
  decide: {
    Deposit: (command, state, ports) => {
      ports.logger.info("Processing deposit", {
  ...
  on: {
    OrderPlaced: {
      ...
      handle: (event, state, ports) => {
        ports.logger.info("Starting fulfillment", {
...
const getAccountById: QueryHandler<BankingInfrastructure, GetAccountQuery> = (
  ...
  ports.logger.debug("Loading account", { id: query.id });
  return ports.accountRepository.findById(query.id);
};
```
...
```ts
Deposit: (command, state, ports) => {
  const log = ports.logger.child("deposit");
...
## Using the Logger in Custom Ports

The `adapters` factory in `DomainWiring` receives the framework logger as its first argument. Use it to inject logging into your custom services at construction time:
