...
- **RabbitMQ**: The [Hotel Booking](/docs/patterns/hotel-booking) sample uses `@noddde/rabbitmq` with a `docker-compose` RabbitMQ container.
- **NATS**: The [Flash Sale](/docs/patterns/flash-sale) sample uses `@noddde/nats` with a TestContainers NATS JetStream container.
