...
The table below shows which database dialects are supported for **persistence** (event store, state store, saga store) and **advisory locking** (pessimistic concurrency) per adapter.

| Dialect    | Drizzle               | Prisma                | TypeORM               |
| ---------- | --------------------- | --------------------- | --------------------- |
| PostgreSQL | ✅ Persistence + Lock | ✅ Persistence + Lock | ✅ Persistence + Lock |
| MySQL      | ✅ Persistence + Lock | ✅ Persistence + Lock | ✅ Persistence + Lock |
| MariaDB    | ✅ Persistence + Lock | ✅ Persistence + Lock | ✅ Persistence + Lock |
| SQLite     | ✅ Persistence only   | ✅ Persistence only   | ✅ Persistence only   |
| MSSQL      | —                     | —                     | ✅ Persistence + Lock |

**Lock** = advisory lock support via `*AdvisoryLocker`. SQLite has no advisory lock mechanism — use `InMemoryAggregateLocker` from `@noddde/engine` for single-process deployments.
