...
Each package exports factory functions or builder APIs that return all persistence implementations wired to share a transaction context:
...
// infra.outboxStore              -- OutboxStore (Drizzle: only when outbox schema provided)
// infra.unitOfWorkFactory        -- UnitOfWorkFactory (real DB transactions)
```

For advanced use cases like per-aggregate state tables, each adapter also provides a [Builder API](#builder-api) (`DrizzleAdapter`, `PrismaAdapter`, `TypeORMAdapter`).
...
For advanced configuration -- per-aggregate state tables, custom column mappings, or selective store setup -- use the `DrizzleAdapter` builder instead of `createDrizzlePersistence`:
...
const adapter = new DrizzleAdapter(db)
  ...
  .withAggregateStateTable("BankAccount", { table: bankAccounts }) // convention-based
  .build();
```

The builder returns a `DrizzleAdapterResult` with all configured stores plus a `stateStoreFor()` accessor for per-aggregate tables:

```ts
adapter.eventSourcedPersistence; // shared event store
adapter.stateStoredPersistence; // shared state store (if configured)
adapter.sagaPersistence; // shared saga store
adapter.unitOfWorkFactory; // UoW factory
adapter.snapshotStore; // optional
adapter.outboxStore; // optional
adapter.stateStoreFor("Order"); // dedicated state persistence for Order
```

All persistence instances from a single builder share the same transaction store, ensuring UoW atomicity across shared and dedicated tables.

The `createDrizzlePersistence` factory still works and delegates to the builder internally -- it's the simpler choice when all aggregates share the same tables.
...
Use `.withAggregateStateTable()` on the builder to map an aggregate to its own table:
...
const adapter = new DrizzleAdapter(db)
  ...
  .withAggregateStateTable("BankAccount", { table: bankAccounts })
  .build();
```

**Column resolution** works in two modes:

1. **Convention-based** (default): The builder scans the Drizzle table definition for columns named `aggregate_id`, `state`, and `version`. If your table follows this naming, no explicit mapping is needed.
...
If convention-based resolution fails (columns not found), `.build()` throws with a clear error listing the available columns in the table.
...
For advanced configuration -- per-aggregate state tables or selective store setup -- use the `PrismaAdapter` builder instead of `createPrismaPersistence`:
...
const adapter = new PrismaAdapter(prisma)
  ...
  .withOutboxStore()
  .withAggregateStateTable("Order", {
    model: "nodddeAggregateState", // or your own Prisma model name
    columns: {
...
The builder returns a `PrismaAdapterResult` with all configured stores plus a `stateStoreFor()` accessor for per-aggregate tables:
...
Since Prisma uses generated model delegates (e.g., `prisma.order`), the `model` field in the config must match the camelCase delegate name on your PrismaClient. The builder validates that the model exists at `build()` time.

The `createPrismaPersistence` factory still works and delegates to the builder internally -- it's the simpler choice when all aggregates share the same tables.
...
For advanced configuration -- per-aggregate state tables, custom entities, or selective store setup -- use the `TypeORMAdapter` builder instead of `createTypeORMPersistence`:
...
const adapter = new TypeORMAdapter(dataSource)
  .withEventStore() // defaults to NodddeEventEntity
  .withSagaStore() // defaults to NodddeSagaStateEntity
  .withSnapshotStore() // defaults to NodddeSnapshotEntity
  .withOutboxStore() // defaults to NodddeOutboxEntryEntity
  .withAggregateStateTable("Order", {
    entity: OrderEntity,
    columns: {
...
Each `with*Store()` method accepts an optional entity class parameter to override the default -- useful if you've customized the noddde entity decorators for your database.

The builder returns a `TypeORMAdapterResult` with all configured stores plus a `stateStoreFor()` accessor for per-aggregate tables:
...
The `createTypeORMPersistence` factory still works and delegates to the builder internally -- it's the simpler choice when all aggregates share the same tables.
