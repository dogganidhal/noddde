...
Provides the store for tracking processed commands. When configured, commands with a `commandId` field are checked for duplicates before execution. See [Idempotent Commands](/docs/running/idempotent-commands) for details.
