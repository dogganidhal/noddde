...
This type becomes the `infrastructure` member of your [AggregateTypes](/docs/modeling/defining-aggregates) bundle. Each component declares only what it needs, and `wireDomain` computes the intersection at compile time.
...
projections: {
  BankAccount: { viewStore: new PrismaBankAccountViewStoreFactory(prisma) },
  TransactionHistory: { viewStore: new InMemoryViewStoreFactory() },
},
```
