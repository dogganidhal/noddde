...
The optional `commandId` field enables [idempotent command processing](/docs/running/persistence#idempotent-command-processing) — when set and an `IdempotencyStore` is configured, duplicate commands are silently skipped.
