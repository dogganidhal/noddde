...
<Mermaid
  chart={`graph LR
  CMD["Command (intent)"] --> AGG["Aggregate (write)"]
  AGG -- "emits" --> EVT["Event (fact)"]
  EVT --> PROJ["Projection (read)"]
  Q["Query (question)"] --> PROJ
  PROJ -- "serves" --> CALLER["Caller"]
`}
/>
...
The optional `commandId` field enables [idempotent command processing](/docs/running/idempotent-commands) — when set and an `IdempotencyStore` is configured, duplicate commands are silently skipped.
...
noddde leverages this pattern throughout the framework. Decide handler maps and evolve handler maps are both keyed by the `name` discriminant, so the framework can automatically narrow the command or event type for each handler.
...
When defining an aggregate, you need to specify several related types: the state shape, the command union, the event union, and the ports type. A naive approach would use positional generics:
...
This means that when you write a decide handler for `AuthorizeTransaction`, TypeScript already knows:
...
decide: {
  ...
  AuthorizeTransaction: (command, state, ports) => {
...
<Mermaid
  chart={`graph TD
  A["1. Payload maps\n{ CreateBankAccount: void;\n  AuthorizeTransaction: ... }"]
  B["2. DefineCommands\nDiscriminated union with\nname + targetAggregateId + payload"]
  C["3. AggregateTypes bundle\n{ state, commands, events, ports }"]
  D["4. defineAggregate\nTyped handler maps:\ncommands + apply"]
  A --> B --> C --> D
`}
/>
...
type InferAggregatePorts<T extends Aggregate> =
...
function applyEvents(events: BankEvent[]): BankState {
  return events.reduce((state, event) => {
    const handler = BankAccount.evolve[event.name];
...
This ensures that TypeScript can distinguish between status values in your decide handlers and evolve handlers.

## How the Framework Uses Messages

Messages are not just data types — they drive the framework's runtime behavior:

1. **Command routing.** When you call `domain.dispatchCommand(command)`, the framework reads `command.name` and routes it to the matching handler in the aggregate's `decide` map.

2. **Event application.** After a decide handler returns events, the framework reads each `event.name` and calls the matching handler in the aggregate's `evolve` map to evolve the state.
