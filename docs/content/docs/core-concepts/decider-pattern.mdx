...
const Auction = defineAggregate<AuctionTypes>({
  ...
  evolve: {
    // ... evolve handlers (no infrastructure — always pure)
  },
});
```

Infrastructure is passed to decide handlers but never to evolve handlers. This is by design: evolve handlers must be pure because they are used to replay events during state reconstruction. If evolve handlers depended on infrastructure, replaying the same events could produce different states depending on external conditions, which would break the event sourcing guarantee.
