...
The Decider pattern in its pure form has no concept of side effects. noddde adds an `infrastructure` field to the types bundle that is injected into decide handlers (for clocks, ID generators, external services) -- but **never** into evolve handlers, which must stay pure so event replay is deterministic. See [Why Are Evolve Handlers Pure?](/docs/design-decisions/why-pure-apply-handlers) for the rationale.
...
noddde implements this pattern with `defineAggregate`, adding type-safe handler maps and infrastructure injection while preserving the pattern's core properties: no base classes, no decorators, no mutation, and full testability. The `decide` and `evolve` maps correspond directly to the Decider pattern's `decide` and `evolve` functions.
