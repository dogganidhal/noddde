...
- **Process Side** -- Sagas react to events and dispatch commands across aggregates, coordinating multi-step workflows that span more than one aggregate (see [Sagas](/docs/process-managers/sagas) for the full pattern).
...
If you need a timestamp or a generated ID, compute it in the **decide handler** (which has access to infrastructure) and include it in the event payload. The evolve handler then reads it from the event, ensuring deterministic replay.

This is why noddde passes infrastructure to decide handlers but **never** to evolve handlers. The type system enforces this boundary:
...
There is no infrastructure parameter. If you try to access infrastructure in an evolve handler, the compiler will prevent it.
