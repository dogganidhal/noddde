...
const bankingDomain = defineDomain({
  writeModel: {
    ...
    standaloneCommandHandlers: {
      SendNotification: async (command, ports) => {
        await ports.notificationService.send(command.payload);
      },
    },
  },
  // ...
...
For a complete treatment of how to define aggregates, commands, and evolve handlers, see [Modeling Your Domain](/docs/modeling/defining-aggregates).
...
const bankingDomain = defineDomain({
  ...
  readModel: {
    ...
    standaloneQueryHandlers: {
      GetSystemStats: async (_query, ports) => {
        return ports.statsService.getStats();
      },
    },
  },
});
...
Communication between the write model, read model, and the outside world flows through three buses. Together they form the `CQRSPorts`:
...
3. **Handle command** -- Pass the reconstructed state, the command, and ports to the decide handler.
...
- You need schema evolution via upcasters or versioned evolve handlers
...
Event sourcing works only if replaying the same events always produces the same state. This imposes a strict requirement: **Evolve handlers must be pure functions**.
...
Evolve handlers must **not** use non-deterministic values:
...
Evolve handlers **should** derive all values from the event payload and current state:
...
If you need a timestamp or a generated ID, compute it in the **decide handler** (which has access to ports) and include it in the event payload. The evolve handler then reads it from the event, ensuring deterministic replay.

This is why noddde passes ports to decide handlers but **never** to evolve handlers. The type system enforces this boundary:
...
There is no ports parameter. If you try to access ports in an evolve handler, the compiler will prevent it.
...
4. Decide handler runs
   AuthorizeTransaction(command, state, ports) -> event
