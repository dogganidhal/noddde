...
`InferProjectionQueryHandler` automatically injects `{ views }` into the infrastructure parameter when your `ProjectionTypes` bundle has a `viewStore` field. See [Type Inference Helpers](/docs/modeling/type-inference#inferprojectioneventhandler-and-inferprojectionqueryhandler) for details.
