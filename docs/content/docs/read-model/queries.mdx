...
A `QueryHandler` receives a query payload and ports, then returns the result:

```ts
type QueryHandler<TPorts extends Ports, TQuery extends Query<any>> = (
...
| `ports`    | `TPorts`              | Ports services (repositories, caches, external clients).                                                            |
...
The handler receives `query` as the payload (`{ id: string }` in this case), destructures the repository from ports, and returns the result.
...
`InferProjectionQueryHandler` automatically injects `{ views }` into the ports parameter when your `ProjectionTypes` bundle has a `viewStore` field. See [Type Inference Helpers](/docs/modeling/type-inference#inferprojectionqueryhandler-and-inferprojectionqueryhandler) for details.
...
```ts
class ProductionQueryBus<TPorts extends Ports> implements QueryBus {
  private handlers = new Map<string, QueryHandler<any, any>>();

  constructor(private ports: TPorts) {}

  register<TQuery extends Query<any>>(
    name: string,
    handler: QueryHandler<TPorts, TQuery>,
  ): void {
  ...
  async dispatch<TQuery extends Query<any>>(
    ...
    return handler(query.payload, this.ports);
  }
}
```
