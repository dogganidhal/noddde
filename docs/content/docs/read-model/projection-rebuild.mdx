...
4. **Replay.** Iterate the full event log (via `EventReader.read()`). Each event is first upcasted (see below). For each event whose name appears in the projection's `on` map: derive `viewId`, load the current view (or use `initialView`), run the reducer, save or delete the new view, tick `onProgress` every `progressInterval` events.
...
Replayed events pass through the owning aggregate's upcaster chain before the reducer sees them, mirroring the aggregate rehydration path. The rebuild reads `event.metadata.aggregateName`, looks up that aggregate's `upcasters` map, and applies the chain registered for the event's name — so a reducer written against the current payload shape never sees a stale payload from an older schema version. Events with no `aggregateName`, or whose aggregate declares no chain for that event name, replay unchanged. See [Event Versioning & Upcasters](/docs/modeling/event-versioning).
...
- **It does not upcast events outside the replay loop.** Upcaster chains are applied on exactly two paths: rebuild replay and aggregate rehydration. Events delivered to projections and sagas through the live event bus — including a broker-backed consumer replaying stored events — arrive at their stored schema version. This is a known limitation, tracked separately from rebuild.
