...
const domain = await wireDomain(definition, {
  projections: {
    Account: { viewStore: (adapters) => infra.accountViewStore },
  },
});
```
...
The factory receives the resolved adapters and should return an **already-initialized** view store instance from it. This keeps the projection definition free of adapter concerns — the view store is created and configured when you set up your ports, not inside domain code. Constructing a view store inline (e.g., `new InMemoryViewStore()`) defeats the purpose of separating domain from adapters.

The factory is **synchronous only**. The view store must be fully initialized before being provided via adapters — async initialization belongs in your adapter setup, not in a wiring factory. The factory is called during `wireDomain()` with the fully resolved adapters (custom + CQRS).
