...
```ts
import type { ViewStore, DefineEvents, DefineQueries, Infrastructure } from "@noddde/core";
...
const AccountProjection = defineProjection<AccountProjectionDef>({
  ...
  // Factory that creates the view store (receives infrastructure for IoC)
...
The `viewStore` field is a factory function, not a direct instance. It receives the resolved infrastructure, enabling inversion of control:

```ts
viewStore: (infra) => new TypeORMAccountViewStore(infra.dataSource),
```

The factory is called during `Domain.init()` with the fully resolved infrastructure (custom + CQRS). It may be async:

```ts
viewStore: async (infra) => {
  const repo = infra.dataSource.getRepository(AccountEntity);
  return new TypeORMAccountViewStore(repo);
},
```
