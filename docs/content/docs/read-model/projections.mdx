...
Every projection is parameterized by a `ProjectionTypes` bundle -- a single named type that captures four dimensions:
...
The `queryHandlers` map contains handlers for serving queries from the projection's view. Each handler receives the query and the infrastructure:
...
Query handlers access data through infrastructure -- typically view repositories that encapsulate the underlying storage. The handler's return type is validated against the `result` type declared in the `DefineQueries` map.
