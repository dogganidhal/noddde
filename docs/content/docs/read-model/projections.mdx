---
title: Projections
description: Building read-optimized views from event streams using reducer maps, query handlers, and defineProjection.
---
...
4. Registered projections receive the events and update their views via reducers
...
The first four fields are required. The `viewStore` field is optional -- when present, it enables automatic view persistence and typed `{ views }` injection into query handlers. See [View Persistence](/docs/read-model/view-persistence) for details.
...
export const BankAccountProjection = defineProjection<BankAccountProjectionDef>(
  {
    ...
    viewStore: (_infra) => new InMemoryViewStore(),
...
- **Exhaustiveness checking** -- TypeScript ensures you handle every event in the union. Missing an event name is a compile error.
- **Can be async** -- Reducers may return `Promise<View>` when they need to perform asynchronous work.
...
2. **Exhaustiveness** -- TypeScript catches missing event handlers at compile time.
...
By default, projection views are stored in an in-memory map per projection name. For production use, noddde provides **view stores** -- typed persistence interfaces for per-entity view management.
...
1. Extracts the view ID from the event via `identity[eventName](event)`
2. Loads the current view from the view store (falling back to `initialView` if not found)
3. Runs the reducer to produce the new view
