...
type MyProjectionDef = {
  ...
  ports: MyInfra; // Dependencies available to query handlers
...
type Infra = InferProjectionPorts<typeof BankAccountProjection>; // BankingInfrastructure
```
  ...
  readModel: {
    projections: {
      ...
      AccountBalance: {
        ...
        viewStore: (adapters) => infra.accountViewStore,
      },
...
The key in the projections map (e.g., `"AccountBalance"`) is the projection's name. It does not need to match the aggregate name. Passing `{ projection, viewStore }` connects the projection to a view store from your ports, enabling automatic view persistence and `{ views }` injection into query handlers.
...
No mocks needed. No ports setup. Just inputs and outputs.
