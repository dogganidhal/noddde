...
Use `defineProjection` to create a projection with full type inference. We recommend extracting each event handler and query handler to its own file, typed with `InferProjectionEventHandler` and `InferProjectionQueryHandler` -- see [Why Extracted Handlers](/docs/design-decisions/why-extracted-handlers). The examples in this section use that pattern.
...
const domain = await wireDomain(bankingDomain, {
  projections: {
    ...
    // TransactionHistory omitted — query-only or uses custom infra.
  },
});
```

The key in the projections map (e.g., `"AccountBalance"`) is the projection's name. It does not need to match the aggregate name. Wiring a `viewStore` factory under `DomainWiring.projections` connects the projection to a view store, enabling automatic view persistence and `{ views }` injection into query handlers.
...
The projection name is a typed union of the projections registered in the enclosing `defineDomain` call — passing an unknown name is a compile-time error. v1 supports eventual-consistency projections only and requires the caller to halt writes during the rebuild window. See [Projection Rebuild](/docs/read-model/projection-rebuild) for the full contract, options, error taxonomy, and adapter requirements.
...
- [Projection Rebuild](/docs/read-model/projection-rebuild) -- Restoring projection views from the event log
