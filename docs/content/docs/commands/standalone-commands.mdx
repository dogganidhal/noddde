...
<Callout type="info">
For **stateful** multi-step workflows that need to track progress, handle failures, and coordinate compensation across aggregates, consider using [Sagas](/docs/sagas/overview) instead. Sagas are event-driven, persist their state, and return commands declaratively — making them testable without mocking buses.
</Callout>
...
## Standalone Commands vs Sagas

noddde provides two mechanisms for cross-aggregate coordination:

| Aspect | Standalone Command Handler | Saga |
|---|---|---|
| Trigger | Command (imperative) | Event (reactive) |
...
| Best for | Simple one-shot orchestration, integrations | Multi-step workflows, long-running processes |

**Use standalone commands** for stateless, simple coordination: sending notifications, syncing with external systems, or dispatching a few commands in sequence.

**Use sagas** when the workflow has multiple steps, needs to track progress, or requires compensation on failure. See the [Sagas guide](/docs/sagas/overview) for details.
...
- When a standalone handler dispatches multiple aggregate commands, consider what happens if one of them fails. For complex failure scenarios, consider a [saga](/docs/sagas/overview) instead.
...
- Learn about [Sagas](/docs/sagas/overview) for stateful, event-driven workflow coordination.
