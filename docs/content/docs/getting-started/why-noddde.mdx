...
| Type safety         | Manual generics on every class     | Inferred from one `Types` bundle       |
...
| Type model           | Commands, events, state declared as separate generics      | One `Types` bundle drives every handler's inference                       |
| Composition          | À-la-carte: assemble the pieces you need                   | `defineDomain` + `wireDomain` as a unified orchestrator                   |
| Sagas / projections  | First-class but wired up externally                        | Defined in the same `Types` ecosystem; saga reactions return commands     |
...
- [Why a single `AggregateTypes` bundle](/docs/design-decisions/why-aggregate-types) — one `Types` bundle instead of five generic parameters
