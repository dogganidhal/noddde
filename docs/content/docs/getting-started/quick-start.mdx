...
type BankAccountDef = {
  ...
  ports: {}; // No custom ports for this example
};
```
...
Define the domain structure, then wire it with in-memory adapters:
...
// Wire with adapters and get a running Domain instance
const domain = await wireDomain(bankingDomain);
```

That is it -- `wireDomain` with no second argument uses in-memory defaults for everything: persistence, buses, and adapters. This is the fastest path to a working domain.
...
3. **Decide handler called** — `decide.AuthorizeTransaction(command, currentState, ports)`
