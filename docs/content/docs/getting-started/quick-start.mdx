...
// 4. The Types bundle — drives all type inference
...
The first command creates the account. The second is declined because the account has no balance, and the framework records a `TransactionDeclined` event. Try changing the merchant or the amount to see how the same command shape flows through `decide` and produces different events.
