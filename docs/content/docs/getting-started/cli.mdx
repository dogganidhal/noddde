...
The CLI is split into three command groups: `noddde new` (alias `noddde n`) for scaffolding new modules, `noddde add` (alias `noddde a`) for extending existing aggregates and projections, and `noddde diagram` (alias `noddde d`) for visualizing the flow.
...
Decide handlers are typed with `InferDecideHandler<Types, "CommandName">` and evolve handlers with `InferEvolveHandler<Types, "EventName">`. Both are standalone exported functions imported into the aggregate definition. This keeps each handler individually testable and the aggregate definition concise.
...
The generated handler is typed with `InferProjectionQueryHandler<Def, "QueryName">` and defaults to `views.load(query.id)` — the handler's first argument is the query payload itself, not a `Query` wrapper. Adjust the payload shape and the handler body to fit your query.
