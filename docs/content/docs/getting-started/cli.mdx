...
Scaffolds a single aggregate with extracted command handlers and apply handlers. Run from the project root — the CLI automatically places files at `src/domain/write-model/aggregates/<name>/`.
...
<Files>
  <Folder name="bank-account" defaultOpen>
    ...
    <Folder name="apply-handlers">
      <File name="index.ts" />
      <File name="apply-bank-account-created.ts" />
    </Folder>
  </Folder>
</Files>

Command handlers are typed with `InferCommandHandler<Def, "CommandName">` and apply handlers with `InferApplyHandler<Def, "EventName">`. Both are standalone exported functions imported into the aggregate definition. This keeps each handler individually testable and the aggregate definition concise.
