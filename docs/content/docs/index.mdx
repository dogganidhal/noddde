...
The shape: every domain concept is an object with handler maps. State transitions are pure functions of an event and a previous state. Infrastructure is passed to handlers as a parameter. Types flow from a single `Types` bundle, end to end.
