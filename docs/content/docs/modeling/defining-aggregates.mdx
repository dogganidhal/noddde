...
Bundle the state, events, commands, and ports into a single `AggregateTypes` type. This is the single type parameter that `defineAggregate` uses for full type inference across all boundaries:

```typescript
import { Ports } from "@noddde/core";
...
The `ports` field specifies what services your decide handlers can access. The base `Ports` type is an empty object -- use it when your aggregate needs no external services. If your handlers need a logger, a clock, or any other service, define a custom interface instead (see [Using Ports](#using-ports) below).
...
- Ports access in decide handlers is correctly typed.
...
When decide handlers need external services -- a clock, a logger, a fraud-check client -- define a custom ports type and include it in the types bundle.
...
```typescript
// types.ts
import { Ports, DefineEvents, DefineCommands } from "@noddde/core";

interface AuctionInfrastructure extends Ports {
  clock: { now(): Date };
}
...
The `PlaceBid` handler destructures `{ clock }` from the ports parameter. `InferDecideHandler` ensures TypeScript knows exactly which services are available:
...
Ports is **never passed to evolve handlers**. Evolve handlers are pure reducers -- they receive only the event payload and the current state. This keeps state evolution deterministic and safe for event replay.
...
- [Decide Handlers](/docs/modeling/command-handlers) -- Patterns for decide handler logic: async handlers, returning multiple events, validation strategies
- [State and Events](/docs/modeling/state-and-events) -- Evolve handler patterns, immutable state updates, and modeling rejection events
