...
We recommend extracting each evolve handler to its own file typed with `InferEvolveHandler` -- see [Why Extracted Handlers](/docs/design-decisions/why-extracted-handlers). The examples in the rest of this page use that pattern.

For a deeper discussion of why evolve handlers are pure, see [Why Are Evolve Handlers Pure?](/docs/design-decisions/why-pure-apply-handlers).
