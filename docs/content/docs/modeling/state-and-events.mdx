...
description: Designing aggregate state for decision-making and writing pure apply handlers that evolve state in response to events.
---
...
The recommended pattern is to extract each apply handler into its own file using `InferApplyHandler`, which derives the full function type from your `AggregateTypes` bundle:
...
<Callout type="info">
  For simple handlers, you can define them inline in the `apply` map. But
  extracted handlers are preferred -- each can be tested in isolation and the
  aggregate definition stays concise.
</Callout>

For a deeper discussion, see [Why Are Apply Handlers Pure?](/docs/design-decisions/why-pure-apply-handlers).
...
```typescript
// apply-handlers/apply-transaction-processed.ts
import type { InferApplyHandler } from "@noddde/core";
import type { BankAccountDef } from "../bank-account";

export const applyTransactionProcessed: InferApplyHandler<
  BankAccountDef,
...
```typescript
// apply-handlers/apply-bid-rejected.ts
import type { InferApplyHandler } from "@noddde/core";
...
Quick reference for the most frequent apply handler shapes, shown as extracted functions with `InferApplyHandler`.
...
- [Command Handlers](/docs/modeling/command-handlers) -- Writing command handlers that enforce business rules and produce events.
