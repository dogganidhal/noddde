...
<Callout type="info">
  noddde injects a framework `Logger` into every handler automatically --
  destructure it from `infrastructure` whenever you need it. See
  [Logging](/docs/running/logging) for the full interface and configuration.
</Callout>
...
export const decidePlaceBid: InferDecideHandler<AuctionTypes, "PlaceBid"> = (
  command,
  state,
  { clock },
) => {
