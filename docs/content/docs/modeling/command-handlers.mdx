...
description: Writing decide handler logic -- validation, returning events, ports access, and type safety
---
...
```typescript
(command: TCommand, state: TState, ports: TPorts) =>
...
| `ports`   | Your domain ports (loggers, clocks, external services)                                        |
...
Throw exceptions only for truly unexpected situations -- programming errors, adapter failures, or invariants that indicate a bug in your code:
...
The rule of thumb: if a human user could cause this condition through normal use of the application (insufficient funds, expired auction, duplicate request), model it as a rejection event. If only a programmer mistake or adapter failure can cause this condition, throw an exception.

## Using Ports

The third parameter gives you access to your domain ports. Destructure the services you need directly in the function signature. `InferDecideHandler` automatically types the ports from your `AggregateTypes` bundle:
...
When a handler needs several port services, destructure them all:
...
Handlers can be asynchronous when they need to call external adapters. Mark the handler with `async` and the framework will `await` it before persisting events:
...
3. **Correct ports type.** The third parameter matches the `ports` field from your `AggregateTypes` bundle. If you declared a `clock` and a `logger`, those are the only services available.
