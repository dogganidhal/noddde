...
We recommend extracting each decide handler to its own file typed with `InferDecideHandler` -- see [Why Extracted Handlers](/docs/design-decisions/why-extracted-handlers). The examples below use that pattern.
