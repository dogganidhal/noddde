...
The recommended pattern is to define each command handler as a standalone function in its own file, typed with `InferCommandHandler`. This gives you full type inference from the `AggregateTypes` bundle without manually reconstructing parameter types:
...
<Callout type="info">
  ...
  the `commands` map. But extracted handlers are preferred as they are
...
```typescript
// command-handlers/handle-close-month.ts
import type { InferCommandHandler } from "@noddde/core";
import type { BankAccountDef } from "../bank-account";

export const handleCloseMonth: InferCommandHandler<
  BankAccountDef,
  "CloseMonth"
> = (command, state) => {
...
The third parameter gives you access to your domain infrastructure. Destructure the services you need directly in the function signature. `InferCommandHandler` automatically types the infrastructure from your `AggregateTypes` bundle:
...
The `commands` map is fully typed. When you use `InferCommandHandler<Def, "CommandName">`, the function signature is derived entirely from your `AggregateTypes` bundle. The framework guarantees the following without requiring any manual type annotations from you:
