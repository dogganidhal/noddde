...
Without upcasting, replaying version 1 events through evolve handlers that expect version 2 payloads will produce incorrect state — the `status` field will be `undefined`.
...
const BankAccount = defineAggregate<BankAccountTypes>({
  ...
  evolve: {
    // evolve handlers always receive the CURRENT version payload.
    // Upcasting happens before evolve handlers are called.
...
During aggregate rehydration (loading events from persistence to rebuild state), the engine applies the upcaster chain to each loaded event before passing it to the evolve handler:
...
4. **Pass upcasted payload** to the evolve handler

New events produced by decide handlers automatically get `metadata.version` set to the current version for their event name.

The same upcasting happens during [projection rebuild](/docs/read-model/projection-rebuild): `Domain.rebuildProjection` resolves each replayed event's `metadata.aggregateName`, applies that aggregate's upcaster chain, and only then invokes the projection's reducer — so rebuilt views are derived from current-shape payloads, exactly as aggregate state is. Live event-bus delivery to projections and sagas does **not** upcast; those handlers receive events at their stored schema version. That is a known limitation of the current version, not an intended asymmetry.
...
Upcaster steps follow the same constraints as evolve handlers:
