...
Load state
  ...
  v
Execute handler
  |  4. commands[name](command, currentState, ports) --> events
  v
Apply events
...
```typescript
handler(command, currentState, ports);
```
...
It is part of `CQRSPorts`, which is provided during domain configuration:
...
Like the `CommandBus`, it is part of `CQRSPorts`.
...
For production deployments requiring durability, distribution, or at-least-once delivery, you can implement the `EventBus` interface with a message broker backend (Kafka, RabbitMQ, NATS) or build composite buses that fan out to multiple destinations. See [Ports](/docs/running/ports) for custom event bus patterns.
...
- [Ports](/docs/running/ports) -- Custom `CommandBus` and `EventBus` implementations, logging decorators, and message broker adapters
