...
Extracts the ports type from an `Aggregate` instance.
...
```typescript
import { InferAggregatePorts } from "@noddde/core";
import { Auction } from "./auction";

type AuctionInfra = InferAggregatePorts<typeof Auction>;
// { clock: { now(): Date } }
```

### When to Use

- **Test setup**: Know exactly which port services to mock
- **Ports factories**: Type helper functions that build the ports object for a specific aggregate
...
`InferProjectionQueryHandler` automatically injects `{ views }` into the ports parameter when your `ProjectionTypes` bundle has a `viewStore` field.

### InferProjectionQueryPorts

Computes the full ports type for a projection's query handlers, conditionally including `{ views }`:

```typescript
import type { InferProjectionQueryPorts } from "@noddde/core";

type Infra = InferProjectionQueryPorts<RevenueDef>;
...
| `InferAggregatePorts<typeof Agg>`    | `Aggregate` instance    | The ports type               |
...
| Helper                                          | Input                    | Extracts                                   |
| ----------------------------------------------- | ------------------------ | ------------------------------------------ |
| `InferDecideHandler<Def, "CmdName">`            | `AggregateTypes` bundle  | Decide handler function type               |
| `InferEvolveHandler<Def, "EventName">`          | `AggregateTypes` bundle  | Evolve handler function type               |
...
| `InferProjectionQueryPorts<Def>`                | `ProjectionTypes` bundle | Conditional ports (with/without `views`)   |
