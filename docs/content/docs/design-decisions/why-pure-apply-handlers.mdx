---
title: "Why Are Evolve Handlers Pure?"
description: Why noddde requires evolve handlers to be pure functions with no ports access.
---

## The Decision

Evolve handlers in noddde are pure functions that receive only the event payload and current state, returning the new state. They have no access to ports, no async capability, and must produce no side effects.
...
If evolve handlers could access ports (databases, APIs, time), event replay becomes non-deterministic:
...
- **Allow ports in evolve handlers** — Some frameworks do this, trading correctness for convenience
...
The decide handler is allowed to be impure (it has ports access). The evolve handler stays pure by working only with data that was captured in the event.
