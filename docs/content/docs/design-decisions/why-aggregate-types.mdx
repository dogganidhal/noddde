...
>(config: { /* ... */ }): Aggregate<TState, TEvents, TCommands, TID, TInfrastructure>;
...
Benefits:
- **Named members** — `state`, `events`, `commands`, `infrastructure` are self-documenting
...
export const BankAccount = defineAggregate<BankAccountDef>({
  initialState: { /* TypeScript knows this must be BankAccountState */ },
  commands: { /* TypeScript knows each handler's parameter types */ },
  apply: { /* TypeScript knows each handler's parameter types */ },
});
```
