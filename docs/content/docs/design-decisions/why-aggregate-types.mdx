...
defineAggregate<BankAccountTypes>({
  /* ... */
});
```
...
- **IDE-friendly** — Hovering over `BankAccountTypes` shows all four members

## Trade-offs

- **One extra type declaration** — You need to declare the `type XxxTypes = { ... }` bundle
