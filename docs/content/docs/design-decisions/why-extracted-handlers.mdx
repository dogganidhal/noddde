...
`InferDecideHandler<Types, "PlaceBid">` derives the full function signature from your `AggregateTypes` bundle. The command is narrowed to the specific variant, the state is typed, and infrastructure is correctly scoped with `FrameworkInfrastructure` merged in. The same pattern applies to all handler kinds:
