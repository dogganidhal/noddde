...
Handlers still receive `CQRSPorts` (including `QueryBus`), so they _can_ read data before deciding which commands to return. The constraint is on outputs, not inputs: you can read from the query bus, but you return commands rather than dispatching them.

For truly imperative side effects (notifications, logging), handlers can call ports methods directly — only command dispatch is declarative.
