...
Use `defineSaga` -- an identity function that provides full type inference. We recommend extracting each `on` entry to its own file typed with `InferSagaOnEntry` -- see [Why Extracted Handlers](/docs/design-decisions/why-extracted-handlers). The examples in this section use that pattern.
