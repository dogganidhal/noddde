...
<Callout type="info">
  ...
  [CLI Reference](/docs/getting-started/cli#noddde-new-saga-name) for details.
</Callout>

<Mermaid
  chart={`graph LR
  subgraph Aggregate
    C["Command"] --> D["decide"] --> E["Event(s)"]
    D --> S1["State"]
  end
  subgraph Saga
    EV["Event"] --> R["react"] --> CMD["Command(s)"]
    R --> S2["State"]
  end
`}
/>
...
| `ports`    | External dependencies available to handlers                      |
...
Use `defineSaga` -- an identity function that provides full type inference. The recommended pattern is to extract each `on` entry into its own file, typed with `InferSagaOnEntry`, then import them into the saga definition. This keeps handlers focused, testable, and easy to navigate.

### Extracted on-entries (recommended)

Each on-entry lives in its own file inside an `on-entries/` directory. The `InferSagaOnEntry` helper derives the exact `{ id, handle }` type from your `SagaTypes` bundle and an event name -- no manual type reconstruction needed.

```ts title="on-entries/on-order-placed.ts"
...
```ts title="on-entries/on-payment-completed.ts"
import type { InferSagaOnEntry } from "@noddde/core";
import type { OrderFulfillmentSagaDef } from "../saga";
...
```ts title="on-entries/on-shipment-delivered.ts"
import type { InferSagaOnEntry } from "@noddde/core";
import type { OrderFulfillmentSagaDef } from "../saga";
...
For simple sagas or entries that are just a few lines, inline handlers are still valid:
...
export const OrderFulfillmentSaga = defineSaga<OrderFulfillmentSagaDef>({
  ...
  startedBy: ["OrderPlaced"],
  on: {
    // Inline is fine for trivial entries
    OrderConfirmed: {
      id: (event) => event.payload.orderId,
      handle: (_event, state) => ({ state }),
    },
    // But extract complex entries to separate files
    OrderPlaced: onOrderPlaced,
    PaymentCompleted: onPaymentCompleted,
  },
});
...
<Callout type="info">
  ...
  `InferSagaOnEntry` are the recommended pattern. Each handler gets its own
  file, its own tests, and a single type annotation that validates the entire
  shape.
</Callout>
...
The `on` field is a map keyed by event name. Each entry has an `id` function that receives the narrowed event type and returns the saga instance ID, and a `handle` function that receives the event, current state, and ports. Whether the entry is inline or extracted, the shape is the same:
...
6. The **`handle` function** is called with `(event, state, ports)`
...
<Mermaid
  chart={`graph LR
  E["Event"] --> ID["on[name].id"] --> LS["Load State"] --> H["on[name].handle"] --> SR["SagaReaction"]
  SR --> PS["Persist State"]
  SR --> DC["Dispatch Commands"]
  DC --> AGG["Aggregate(s)"]
  AGG --> NE["New Event(s)"]
  NE -. "cycle repeats" .-> E
`}
...
type Entry = InferSagaOnEntry<OrderFulfillmentSagaDef, "OrderPlaced">;
```
...
type Infra = InferSagaPorts<typeof OrderFulfillmentSaga>;
type Id = InferSagaId<typeof OrderFulfillmentSaga>; // string
```

See [Type Inference Helpers](/docs/modeling/type-inference#handler-level-inference) for the full list of handler-level and definition-level helpers.
...
| **Output**         | Side effects via ports                      | Declarative command returns                  |
