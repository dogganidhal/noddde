...
depends_on: [id, edd/event, cqrs/command/command, ports/index]
docs:
  - sagas/overview.mdx
...
- **`SagaTypes`** is a type with four required fields:
  ...
  - `ports: Ports` -- external dependencies for event handlers.
...
- **`SagaEventHandler<TEvent, TState, TCommands, TPorts>`** is a function type:

  - Parameters: `(event: TEvent, state: TState, ports: TPorts & CQRSPorts & FrameworkPorts)`.
  ...
  - Ports is merged with `CQRSPorts` and `FrameworkPorts` via intersection (providing bus access and logger).

- **`SagaOnEntry<TEvent, TState, TCommands, TPorts, TSagaId>`** is an object type that bundles identity extraction and handler for one event:

  - `id: (event: TEvent) => TSagaId` -- extracts the saga instance ID from the event. Required.
  - `handle: SagaEventHandler<TEvent, TState, TCommands, TPorts>` -- the saga event handler.
...
- **Infer utilities** (operate on `Saga` definition instances):
  ...
  - `InferSagaPorts<T extends Saga>` = inferred `U["ports"]`.
...
- **Handler-level inference utilities** (operate on `SagaTypes` bundle, for typing extracted handlers in separate files):
  ...
  - `InferSagaEventHandler<T extends SagaTypes, K extends T["events"]["name"]>` = `SagaEventHandler<Extract<T["events"], { name: K }>, T["state"], T["commands"], T["ports"]>`. Resolves to the exact saga event handler function type for event `K`, with the event narrowed via `Extract`, and ports merged with `CQRSPorts` and `FrameworkPorts` (via `SagaEventHandler`).

  - `InferSagaOnEntry<T extends SagaTypes, K extends T["events"]["name"], TSagaId extends ID = string>` = `SagaOnEntry<Extract<T["events"], { name: K }>, T["state"], T["commands"], T["ports"], TSagaId>`. Resolves to the full `{ id, handle }` bundle for event `K`, with a customizable saga ID type.

## Behavioral Requirements

- Saga event handlers receive the FULL event object (with narrowed type), the current saga state, and ports merged with CQRS buses and framework ports (logger).
...
- `InferSagaEventHandler<T, K>` resolves to a function receiving the narrowed event (via `Extract<T["events"], { name: K }>`), the saga state, and ports merged with `CQRSPorts` and `FrameworkPorts`, returning `SagaReaction` or `Promise<SagaReaction>`.
...
- Ports parameter in handlers always includes `CQRSPorts` and `FrameworkPorts` via `&`.
...
### SagaEventHandler receives full event and merged ports
...
describe("SagaEventHandler", () => {
  ...
  it("should receive ports merged with CQRSPorts and FrameworkPorts", () => {
    expectTypeOf<Parameters<Handler>[2]>().toEqualTypeOf<
      MyInfra & CQRSPorts & FrameworkPorts
    >();
  });
...
import type { DefineEvents, Command, Ports } from "@noddde/core";
import { defineSaga } from "@noddde/core";
...
describe("Saga Infer utilities", () => {
  ...
  it("should infer ports type", () => {
    expectTypeOf<InferSagaPorts<typeof saga>>().toEqualTypeOf<MyInfra>();
  });
...
```ts
import { describe, it, expectTypeOf } from "vitest";
import type { DefineEvents, Command, Ports, InferSagaId } from "@noddde/core";
import { defineSaga } from "@noddde/core";
...
### InferSagaEventHandler narrows event with merged ports
...
describe("InferSagaEventHandler", () => {
  ...
  interface FulfillmentInfra extends Ports {
    notifier: { notify(msg: string): void };
  }
  ...
  it("should merge ports with CQRSPorts and FrameworkPorts", () => {
    type Handler = InferSagaEventHandler<FulfillmentTypes, "OrderPlaced">;
    expectTypeOf<Parameters<Handler>[2]>().toEqualTypeOf<
      FulfillmentInfra & CQRSPorts & FrameworkPorts
    >();
  });
