...
- **`EventMetadata`** is an interface with:
  ...
  - `causationId: string` — ID of the command or event that directly caused this event. Must be a per-dispatch identifier (the causing command's `commandId` or the causing event's `eventId`) — never a static command/event name (see Behavioral Requirement 8).
...
8. `causationId` must be a per-dispatch identifier, never a static command/event name. Populating it from `command.commandId` (auto-generated by the engine per dispatch when the caller omits one) or a causing event's `eventId` is compliant; falling back to the command's `name` string is not, since it collapses every dispatch of the same command type into one indistinguishable value and defeats the field's audit/debugging purpose. The type system does not enforce this — it is `string` — so this is a documentation contract the engine must honor. See `specs/api-freeze.spec.md` decision 4.
