...
> `ApplyHandler` is a pure, synchronous function type that evolves aggregate state in response to an event. It is the replay engine of event sourcing -- deterministic and free of side effects so that replaying events always produces the same state.

## Type Contract

- **`ApplyHandler<TEvent, TState>`** is a function type:
...
- No infrastructure parameter is present -- apply handlers are deliberately isolated from I/O.
...
- **State type is a primitive**: `ApplyHandler<SomeEvent, number>` is valid; returns a `number`.
...
- `ApplyHandler` is used in `ApplyHandlerMap` inside the `Aggregate` interface, where one handler is required per event name.
...
import type { ApplyHandler, DefineEvents } from "@noddde/core";
...
```ts
import { describe, it, expectTypeOf } from "vitest";
import type { ApplyHandler, Event } from "@noddde/core";

describe("ApplyHandler synchronous constraint", () => {
...
describe("ApplyHandler with primitive state", () => {
