...
7. **Consumer subscription** -- When `connect()` is called, the consumer subscribes to the topic `${topicPrefix}${eventName}` for each registered event name. Registering an additional handler via `on()` after `connect()` is supported **only** for an event name whose topic is already subscribed (this simply appends another handler for in-process fan-out). Calling `on()` after `connect()` for an event name whose topic is **not** already subscribed **throws** an `Error`, because kafkajs forbids subscribing to a new topic on a running consumer (`Cannot subscribe to topic while consumer is running`). The thrown message instructs the caller to register all handlers before `connect()`. The adapter does **not** attempt (and does not pretend) to subscribe late: there is no silent-loss path and no misleading "will be retried" log.
...
- **on() called after connect() for a new topic**: Throws an `Error` — kafkajs cannot subscribe to a new topic on a running consumer, so the subscription would be silently lost. The caller must register all handlers before `connect()`.
