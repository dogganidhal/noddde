...
export function defineScaleContract(
  ...
  describe.skipIf(!slowTestsEnabled())(`ScaleContract: ${adapterLabel}`, () => {
    ...
    it(`loadUnpublished(${OUTBOX_ENTRIES}) returns every entry within budget`, async () => {
    ...
    it(`markPublishedByEventIds over ${OUTBOX_ENTRIES} entries marks exactly the targeted ${MARK_SAMPLE}`, async () => {
      for (let start = 0; start < OUTBOX_ENTRIES; start += SAVE_BATCH) {
        const batch: OutboxEntry[] = [];
        for (let i = start; i < start + SAVE_BATCH; i++) {
          batch.push({
      ...
      const stillUnpublished = await ctx.outbox.loadUnpublished(OUTBOX_ENTRIES);
      expect(stillUnpublished).toHaveLength(OUTBOX_ENTRIES - MARK_SAMPLE);
      expect(elapsed).toBeLessThan(MARK_PUBLISHED_BUDGET_MS);
    });

    it(`load() of a ${AGGREGATE_EVENTS}-event aggregate completes within budget`, async () => {
      let version = 0;
