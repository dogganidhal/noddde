...
describe("generateTypeORMMigration", () => {
  it("should generate default shared tables for sqlite", () => {
    const sql = generateTypeORMMigration("sqlite");
  ...
  it("should use MySQL types for mysql dialect", () => {
    const sql = generateTypeORMMigration("mysql");
  ...
  it("should use MSSQL types for mssql dialect", () => {
    const sql = generateTypeORMMigration("mssql");

    expect(sql).toContain("INT IDENTITY(1,1) PRIMARY KEY");
    expect(sql).toContain("NVARCHAR(MAX) NOT NULL");
    expect(sql).toContain("NVARCHAR(255) NOT NULL");
  });
});
