...
Aggregates are consistency boundaries — the units that process commands and enforce business rules. In noDDDe, an aggregate is defined using the functional [Decider pattern](/docs/concepts/decider-pattern):
...
```ts
const BankAccountView: ProjectionV2<BankAccountEvent, ViewType> = {
  reducer: (view, event) => { /* build view from events */ },
};
```

### Domain Configuration

`configureDomain` wires aggregates, projections, and infrastructure into a running domain:
...
const domain = await configureDomain<MyInfrastructure>({
  ...
  infrastructure: { /* persistence, buses, custom infra */ },
});
```
...
- Flexible persistence (event sourcing or state storage)
- A functional approach to DDD patterns

## Next Steps

- [Installation](/docs/getting-started/installation) — Install `@noddde/core`
- [Quick Start](/docs/getting-started/quick-start) — Build your first aggregate
- [Concepts](/docs/concepts/messaging) — Understand the foundations
